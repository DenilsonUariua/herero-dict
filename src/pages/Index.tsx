...
const Index = () => {
  ...
  return (
    <>
      <MetaTags
        title="Otjiherero Dictionary - Discover the Otjiherero Language"
      ...
      <main className="min-h-screen p-6 space-y-6">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-7xl mx-auto">
          {words.map((word, index) => (
            <WordCard key={word.$id ?? index} {...word} />
          ))}
          {showMessage()}
        </section>
        ...
        <footer className="flex items-center justify-center fixed bottom-1 font-thin right-4">
          <MessagePopup />
          <p className="text-xs">Developed by <a className="underline" href="https://github.com/DenilsonUariua" target="_blank" rel="noreferrer">Tjarirove</a></p>
        </footer>
      </main>
    </>
  );
